...
| **Not covered** | Omni Platform (4WD) — see the [Omni Platform specification](https://georgemandokoro.github.io/WHILL_Serial_API/omni/spec/) |
...
| `0x09` | [SetDeviceLock](#49-setdevicelock-0x09) | Lock / unlock the mobility device | none |
| `0x0A` | [SetJoystickPause](#410-setjoystickpause-0x0a) | Pause / resume joystick driving | none |
| `0x0B` | [SetMaxSpeedLevel](#411-setmaxspeedlevel-0x0b) | Limit the selectable speed level | none |
| `0x0C` | [SetSpeedLevel](#412-setspeedlevel-0x0c) | Select the speed level | none |
| `0x0D` | [SoundHorn](#413-soundhorn-0x0d) | Sound the horn | none |
| `0x0E` | [SetAutoPowerOff](#414-setautopoweroff-0x0e) | Enable / disable auto power off | none |
| `0x0F` | [GetSettings](#415-getsettings-0x0f) | Read the current value of every setting | [0x0F](#62-settings-response) |
...
> ⚠️ `D0` is tested for equality with `1`, so any value other than `1` — not just `0` — is treated as **unlock** rather than being rejected. A corrupted or mistyped parameter therefore unlocks. Send exactly `01` to lock, and confirm the result with [`GetSettings`](#415-getsettings-0x0f).
...
>
> This is unrelated to [`SetDeviceLock`](#49-setdevicelock-0x09), which locks the mobility device itself.

The current state is reported in [data set 1](#52-data-set-1--live-state) byte 3 (`JOYSTICK_PAUSE`).

### 4.11 SetMaxSpeedLevel (`0x0B`)
...
The current value is reported in [data set 1](#52-data-set-1--live-state) byte 5 (`SPEED_LEVEL`). That field reports the **selected** level and is not reduced by [`SetMaxSpeedLevel`](#411-setmaxspeedlevel-0x0b); the level actually in effect is byte 26 (`SPEED_MODE_INDICATOR`).
...
| [`SetDeviceLock`](#49-setdevicelock-0x09) | `DEVICE_LOCK` |
| [`SetJoystickPause`](#410-setjoystickpause-0x0a) | `JOYSTICK_PAUSE` |
| [`SetMaxSpeedLevel`](#411-setmaxspeedlevel-0x0b) | `MAX_SPEED_LEVEL` |
| [`SetSpeedLevel`](#412-setspeedlevel-0x0c) | `SPEED_LEVEL` |
| [`SetAutoPowerOff`](#414-setautopoweroff-0x0e) | `AUTO_POWER_OFF` |
...
| 2 | `DEVICE_LOCK` | Device lock state set by [SetDeviceLock](#49-setdevicelock-0x09). `1` = locked, `0` = unlocked |
| 3 | `JOYSTICK_PAUSE` | Joystick pause state set by [SetJoystickPause](#410-setjoystickpause-0x0a). `1` = paused, `0` = resumed |
| 4 | `MAX_SPEED_LEVEL` | Max usable speed level (**1–4**) set by [SetMaxSpeedLevel](#411-setmaxspeedlevel-0x0b) |
| 5 | `SPEED_LEVEL` | Speed level (**1–4**) currently selected on the WHILL — [Note 1](#note-1--speed_level) |
| 6 | `AUTO_POWER_OFF` | Auto power off state set by [SetAutoPowerOff](#414-setautopoweroff-0x0e). `1` = enabled, `0` = disabled |
...
The speed level currently selected on the WHILL, matching the speed indicator LED. It is **1-based** (`1`–`4`) and reports the **selected** value: it is not reduced when [`SetMaxSpeedLevel`](#411-setmaxspeedlevel-0x0b) caps the speed. To know the level actually in effect, use `SPEED_MODE_INDICATOR` (byte 26).
...
Reply to [`GetSettings`](#415-getsettings-0x0f).
...
Reply to [`GetCapability`](#416-getcapability-0x10).
